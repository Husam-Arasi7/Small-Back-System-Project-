...
enum enATMOptions { enQuickWithdraw = 1, enNormalWithdraw = 2, enDeposit = 3, enCheckBalance = 4, enUpdatePinCode = 5 ,  enLogout = 6 };
...
void SaveCurrentClient() {
    vector <stClient> vCleints = LoadClientsDateFromFile(ClientsFileName);
    ...
    for (stClient& c : vCleints) {
        ...
        SaveClientDataToFile(ClientsFileName, vCleints);
}




void UpdateCurrentClientPinCode() {
    ...
    } while (OldPin != CurrentClient.PinCode);
...
short ReadATMOptions() {
    short Choose;

    cout << "Please select one of them [1] to [6] ? " << endl;
    cin >> Choose;
...
void ShowATMMainMenue() {
    ...
    cout << "5. Update PinCode" << endl;
    cout << "6. Logout" << endl;
    cout << "======================================================" << endl;

    
    PerformATMMainMenu((enATMOptions)ReadATMOptions());
}
